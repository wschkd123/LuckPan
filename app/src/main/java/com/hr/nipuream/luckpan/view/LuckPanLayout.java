...
public class LuckPanLayout extends RelativeLayout {
    ...
    private int mScreenWidth, mScreeHeight;
    ...
    public LuckPanLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mScreeHeight = getResources().getDisplayMetrics().heightPixels;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mMinValue = Math.min(mScreenWidth, mScreeHeight);
        mMinValue -= Util.dip2px(context, 10) * 2;
        Logger.getLogger().d("screenWidth = " + mScreenWidth + "screenHeight = " + mScreeHeight + "MinValue = " + mMinValue);
        setMeasuredDimension(mMinValue, mMinValue);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(mCircleX, mCircleY, radius, backgroundPaint);
    ...
    private void drawSmallCircle(boolean firstYellow) {
        ...
        for (int i = 0; i <= 360; i += 20) {
            int x = (int) (pointDistance * Math.sin(Util.change(i))) + mCircleX;
            int y = (int) (pointDistance * Math.cos(Util.change(i))) + mCircleY;
